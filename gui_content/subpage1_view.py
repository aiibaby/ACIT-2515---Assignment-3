...
class SubPage1:
    ...
    def __init__(self, type, value, exist_value):
        ...
        if self.type == 2:
            self.tk.title("Find Temperature Reading")
            ...
            self.tk.title("Update Temperature Reading")
            self.id_label = tkinter.Label(self.tk, text="ID", width=15)
            self.id_label.place(x=10, y=10)
            self.id_entry = tkinter.Entry(self.tk, width=23)
            self.id_entry.place(x=110, y=10)
            ...
            self.tk.title("Add Temperature Reading")
            self.date_label = tkinter.Label(self.tk, text="Timestamp", width=15)
        ...
        if self.type == 2:
            ...
            try:
                self.code = requests.get(
                    f"http://127.0.0.1:5000/sensor/temperature/reading/{self.id_entry.get()}").status_code
                response = requests.get(
                    f"http://127.0.0.1:5000/sensor/temperature/reading/{self.id_entry.get()}").json()
            ...
            self.code = requests.put(f"http://127.0.0.1:5000/sensor/temperature/reading/{self.id_entry.get()}",
            ...
            headers = {"content-type": "application/json"}
            self.code = requests.post("http://127.0.0.1:5000/sensor/temperature/reading", json=self.add_message,
